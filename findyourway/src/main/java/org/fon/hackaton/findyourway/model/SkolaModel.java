...
public class SkolaModel {
	...
	private LinkedList<PodrucjeRada> vratiPodrucjaRada(int idSkole) {
		ApiKomunikacija api = new ApiKomunikacija();
		String result;
		LinkedList<PodrucjeRada> podrucja = new LinkedList<>();
		try {
			result = api.sendGet(urlPodaciOSrednjim);
			Gson gson = new GsonBuilder().create();
